...
Matrices or more generally arrays are multi-dimensional generalizations of vectors. Like vectors, they are homogeneous. In fact, they are vectors that have a dimension attribute `dim`. They can be indexed by two or more indices and will be printed in special ways.

A Matrix is a 2-dimensional data structure. You can create a matrix using the `matrix()` function, or by combining vectors using `cbind()` (column bind) or `rbind()` (row bind).
