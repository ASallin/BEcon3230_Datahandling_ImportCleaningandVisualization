...
- Replace the middle value of `my_matrix` (second column, second row) with the numeric 10.

### Sum it up!

With a for-loop, loop through each column of `my_matrix` and compute the sum as well as the mean of each column.

**Advanced**: repeat the task with the R function `apply`. Remember, this function takes a matrix, a margin (1 for rows, 2 for columns), and a function to apply.
