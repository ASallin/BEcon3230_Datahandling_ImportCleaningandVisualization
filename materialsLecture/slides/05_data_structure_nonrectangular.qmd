...
subtitle: 'Lecture 5: Data Structures: Non-rectangular Data'
date: "2026-10-15"
echo: false
...
## Goals of last lecture: from text to data
...
A csv-parser takes a text file with CSV standard structure as input and builds a data structure in the form of a table.
```{}
...
- 🌶️🌶️`dim(dataCHframe[, dataCHframe$PartyLeft > 40])` returns the same as `dim(dataCHframe[, c(2,3)])`
- `dim(dataCHframe[dataCHframe$PartyLeft > 40 | dataCHframe$PartyLeft < 40, ])` returns `c(3,3)`
- `dataCHframe` is a data.frame, which is a list consisting of one named character vector and two named numeric vectors.
...
<br>
  ...
  - start tag: **`<geo_place_name>`**`Coney Island (CD13)</geo_place_name>`
  - data value:	`<geo_place_name>`**`Coney Island (CD13)`**`</geo_place_name>`
  - end tag:	`<geo_place_name>Coney Island (CD13)`**`</geo_place_name>`**
