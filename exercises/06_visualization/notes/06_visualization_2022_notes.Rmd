...
First, read the csv directly from the John Hopkins University github page. If you are working from behind a firewall or without admin privileges, you might not be able to load the csv directly. In this case, you should open the webpage, copy its content, paste it in a text file, save it with the .csv extension, and import it as usual.
...
Second, we need to make the dataset LONG instead of WIDE. 
...
Third, we need to convert the variable `date` from character to date format. After this, we compute the cumulative number of cases per country over the time, starting from the first day of the pandemic. To compute the cumulative number of cases, we first arrange (order) the dataset per country and per date, and we then use the function `cumsum`. As the cumulative sum is a large integer, R will return it as NA (see note in code). Make sure you convert integers to numeric values.
...
Last step: we need to restrict our dataset to the countries of interest. Our dataset is then ready to plot. 
...
We now plot the data using `ggplot`. We convert the number of cases to logs directly in the plot using the command `scale_y_continuous`.
```{r echo = TRUE, message = FALSE, warning=FALSE}
# Plot
ggplot(
